...
class leomenusidebar extends Module {
    ...
    public function __construct() {
        ...
        $this->description = $this->l('Leo  Megamenu Silebar Support Leo Framework Version 3.0');
        $this->Languages();
	...
	public function install()
	{
		/* Adds Module */
		if (parent::install() &&
			$this->registerHook('leftColumn') && $this->registerHook('displayTop') && $this->registerHook('displayTopColumn') && $this->registerHook('displayNav') && $this->registerHook('displaySlideshow') && Configuration::updateValue('sbmenu_iscache', 1)
                && Configuration::updateValue('sbmenu_cachetime', 24) && Configuration::updateValue('LEO_MENUSIDEBAR_TYPE', 'right'))
		{
			$res = true;
...
//			}
    ...
    public function getConfigFieldsValues( $obj ) {      
        ...
        $this->image_base_url = Tools::htmlentitiesutf8($protocol . $_SERVER['HTTP_HOST'] . __PS_BASE_URI__) . 'themes/'.$this->themeName.'/img/modules/leomenusidebar/icons/';
	...
	public function processHook($hook=null){
		global $cookie;
		 $this->context->controller->addCSS( $this->_path . 'leomenusidebar.css', 'all' );
                 ...
                 if (file_exists(_PS_ALL_THEMES_DIR_.$themeName.'/modules/'.$this->name.'/views/templates/hook/'.$hook.'/megamenu.tpl')) {
		 ...
		 if (!$this->isCached($tpl, $this->getCacheId()))
			...
			if($typesub == 'auto'){
					...
					if($hook && $hook == "rightcolumn"){
						if(isset($_COOKIE[$theme.'_layout_dir']) && $_COOKIE[$theme.'_layout_dir']){
							$layout = $_COOKIE[$theme.'_layout_dir'];
    ...
    public function getWidgetContent( $id, $type, $data, $showWidgetID=1){
        ...
        $idText = "";
        if($showWidgetID) $idText = ' id="wid-'.$id.'"';
