...
class LeoWidgetSlideshow extends LeoWidgetBase {
    ...
    public function checkFolderImage() {
        ...
        if (file_exists($pathimg) && is_dir($pathimg) && file_exists($paththum) && is_dir($paththum))
            return;
        @mkdir(_PS_ALL_THEMES_DIR_ . $this->themeName . '/img/modules/', 0755, true);
        @mkdir(_PS_ALL_THEMES_DIR_ . $this->themeName . '/img/modules/' . $this->name . '/', 0755, true);
        @mkdir($pathimg, 0755, true);
        @mkdir($paththum, 0755, true);
    }
